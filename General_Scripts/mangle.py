...
def my_norm(Spec,pbs,mag,name=None):
    ...
    m = Syn_mag(pbs,spec)
    m=m[list(pbs.keys())[0]]
    factor = 10**(-2/5*(float(mag)-float(m)))
...
# filename = 'sn2019vx_20200218_131507_mmt_v1.ascii'

# Spec = S.FileSpectrum(folder + filename)
# pbs_tess = load_pbs(['TESS/TESS.Red'], 0, model='AB',SVO = True)
# pbs_psr = load_pbs(['PAN-STARRS/PS1.r'], 0, model='AB',SVO = True)
# spec = my_norm(Spec, pbs_psr, 15.1751, name=None)
